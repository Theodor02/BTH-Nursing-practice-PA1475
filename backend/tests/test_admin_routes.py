...
def test_categories_nested_returns_categories_with_active_field(client, monkeypatch):
    ...
    fake_q = types.SimpleNamespace(id="q1", question_number=1, template="A short question", active=True)
    ...
    assert data[0] == {
        ...
        "questions": [{"id": "q1", "question_number": 1, "excerpt": "A short question", "active": True}],
    }
...
def test_categories_nested_truncates_long_question_excerpts(client, monkeypatch):
    ...
    long_template = "A" * 60
    fake_q = types.SimpleNamespace(id="q2", question_number=1, template=long_template, active=False)
    fake_cat = types.SimpleNamespace(
...
def test_get_entity_course_returns_fields_including_active(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/0/3")
...
def test_get_entity_course_returns_404_when_not_found(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/0/99")
...
def test_get_entity_category_returns_fields_including_active(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/1/5")
...
def test_get_entity_category_returns_404_when_not_found(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/1/99")
...
def test_get_entity_question_returns_full_template(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/2/1")
...
def test_get_entity_question_returns_404_when_not_found(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/2/9999")
...
def test_get_entity_unit_returns_unit_fields(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/3/2")
...
def test_get_entity_unit_returns_404_when_not_found(client, monkeypatch):
    ...
    response = client.get("/api/admin/entity/3/99")
...
def test_get_entity_returns_400_for_unknown_entity_type(client):
    response = client.get("/api/admin/entity/9/1")
...
def test_get_entity_returns_400_for_non_integer_id_on_numeric_entity(client):
    response = client.get("/api/admin/entity/0/not-a-number")
...
def test_get_questions_returns_paginated_list(client, app_module, monkeypatch):
    fake_templates = [
        types.SimpleNamespace(id="q1", question_number=1, template="Short template", unit="kg", active=True),
        types.SimpleNamespace(id="q2", question_number=None, template=None, unit=None, active=False),
    ]
    monkeypatch.setattr(
    ...
    data = response.get_json()
    assert len(data) == 2
    assert data[0] == {"id": "q1", "question_number": 1, "excerpt": "Short template", "unit": "kg", "active": True}
    assert data[1] == {"id": "q2", "question_number": None, "excerpt": None, "unit": None, "active": False}


def test_get_questions_truncates_long_template_excerpts(client, app_module, monkeypatch):
    long_text = "B" * 60
    fake_templates = [
        types.SimpleNamespace(id="q3", question_number=1, template=long_text, unit="L", active=True),
    ]
    monkeypatch.setattr(
        app_module.fake_db.session, "execute",
        lambda *a, **kw: _fake_execute_returning(fake_templates),
...
def test_get_questions_rejects_non_integer_limit(client):
    response = client.get("/api/admin/questions?limit=abc")
...
def test_get_questions_rejects_non_integer_offset(client):
    response = client.get("/api/admin/questions?offset=xyz")
...
def test_get_questions_caps_limit_at_200(client, app_module, monkeypatch):
    ...
    response = client.get("/api/admin/questions?limit=9999")
...
def test_mutate_rejects_non_list_body(client):
    response = client.post("/api/admin/mutate", json={"not": "a list"})
...
def test_mutate_processes_multiple_operations_in_one_batch(client):
    ...
    response = client.post("/api/admin/mutate", json=operations)
...
def test_mutate_question_create_without_course_ids_succeeds(client):
    response = client.post(
        "/api/admin/mutate",
...
def test_mutate_question_edit_returns_success(client, monkeypatch):
    import routes.admin_routes as ar

    fake_qt = types.SimpleNamespace(
        id=1, template="Old text", variables=None, formula=None,
    )
    monkeypatch.setattr(ar.db.session, "get", lambda *a, **k: fake_qt)

    response = client.post(
        "/api/admin/mutate",
        json=[{"type": 2, "action": 2, "body": {"id": 1, "template": "New text"}}],
    )
...
def test_mutate_question_edit_requires_id(client):
    ...
    result = response.get_json()["results"][0]
    assert result["status"] == "error"
    assert "id" in result["message"] and "required" in result["message"]


def test_mutate_question_archive_returns_success(client):
    response = client.post(
        "/api/admin/mutate",
        json=[{"type": 2, "action": 1, "body": {"id": 1}}],
    )
...
def test_set_admin_promotes_user(client, monkeypatch):
    ...
    response = client.post("/api/admin/users/set_admin", json={"email": "teacher@bth.se", "is_admin": True})
...
def test_set_admin_revokes_admin(client, monkeypatch):
    ...
    response = client.post("/api/admin/users/set_admin", json={"email": "teacher@bth.se", "is_admin": False})
...
def test_set_admin_returns_404_when_user_not_found(client, monkeypatch):
    ...
    response = client.post("/api/admin/users/set_admin", json={"email": "nobody@bth.se"})

    assert response.status_code == 404


def test_set_admin_returns_400_when_email_missing(client):
    response = client.post("/api/admin/users/set_admin", json={})
...
def test_set_admin_returns_400_when_is_admin_not_bool(client):
    response = client.post("/api/admin/users/set_admin", json={"email": "teacher@bth.se", "is_admin": "yes"})
